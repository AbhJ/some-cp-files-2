//Coded by Abhijay Mitra (AbJ) on 2021 / 02 / 12 in 13 : 45 : 50
//title - D_Restore_Permutation.cpp
#include <iostream>
#include <stdlib.h>
#include <algorithm>
#include <cstdio>
...
#define F first
#define S second
int n, a[N], b[N], vis[N], tot, A[N], t[4 * N];
...
void solve(){
	...
	build(A);
	per(i, n, 1){
		//find out the position of current num
		int l = - 1, r = n - 1, m;
		while(r - l > 1){
			m = ((l + r) >> 1);
			if(sum(1, m) >= a[i])
		...
		b[i] = *s.lower_bound(l + 2);
		//after l - 1 there shall be some crosses
		update(b[i], 0);
		s.erase(s.find(b[i]));
	}
	rep(i, 1, n)
