...
void solve(){
    int m;cin>>m;std::vector<int> v(m);
    for (int i = 0; i < m; ++i)
    {
        cin>>v[i];if(i)v[i]+=v[i-1];
    }
    int x,y;cin>>x>>y;
    if(m>2)for (int i = 0; i < m; ++i)
    {
        if(v[i]>=x)if((v[i]<=y))if((v[m-1]-v[i])>=x)if((v[m-1]-v[i])<=y){cout<<i+2;return;}
    }
    if(m==2)if((v[0]>=x) and v[0]<=y)if(v[1]-v[0]>=x)if(v[1]-v[0]<=y){cout<<1;return;}
    cout<<0;
}
int main()
{   
